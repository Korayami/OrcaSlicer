...
;    }
    this->Bind(wxEVT_CHAR_HOOK, [this](wxKeyEvent &evt) {
        ...
        if (evt.CmdDown() && evt.RawControlDown() && evt.GetKeyCode() == 'F') {
            ...
            return;}
#endif
        if (evt.CmdDown() && evt.GetKeyCode() == 'R') { if (m_slice_enable) { wxGetApp().plater()->update(true, true); wxPostEvent(m_plater, SimpleEvent(EVT_GLTOOLBAR_SLICE_PLATE)); this->m_tabpanel->SetSelection(this->m_tabpanel->GetSelection() == tpLiveView ? tpLiveView : tpPreview); } return; }
    ...
    m_slice_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event)
        {
            ...
            if (slice) {
                ...
                this->m_tabpanel->SetSelection(this->m_tabpanel->GetSelection() == tpLiveView ? tpLiveView : tpPreview);
            }
        });
